...
const basePath = '/kernel/cyber-survivor';
...
const trackLocation = (req, res, next) => {
    if (req.user && req.user.teamId && req.user.delegateId) {
        User.updateOne(
            ...
            { $set: { "delegates.$.lastKnownLocation": req.originalUrl.replace(basePath, '') } }
...
const authorizeRole = (requiredRole) => {
    return (req, res, next) => {
        if (req.user && req.user.role === requiredRole) {
            ...
            res.status(403).redirect(`${basePath}/dashboard`);
        }
    };
};
...
router.post('/api/logout', protectPlayerRoute, async (req, res) => {
    try {
...
router.get('/api/get-progress', protectPlayerRoute, async (req, res) => {
    try {
...
router.post('/api/check-answer', protectPlayerRoute, async (req, res) => {
...
router.post('/api/end-mission', protectPlayerRoute, async (req, res) => {
...
router.post('/api/get-hint', protectPlayerRoute, async (req, res) => {
...
router.post('/api/submit-final-challenge', protectPlayerRoute, async (req, res) => {
...
// ======================= PAGE ROUTES (on router) =======================
router.get('/', checkAuthStatus, (req, res) => res.render('index', { title: 'Cyber Survivor', isLoggedIn: res.locals.isLoggedIn, basePath }));
router.get('/login', redirectIfLoggedIn, (req, res) => res.render('login', { title: 'Login', basePath }));
router.get('/register', redirectIfLoggedIn, (req, res) => res.render('register', { title: 'Register', basePath }));
router.get('/dashboard', protectPlayerRoute, async (req, res) => {
    try {
        ...
        if (delegate && delegate.lastKnownLocation && delegate.lastKnownLocation !== '/dashboard') {
            return res.redirect(basePath + delegate.lastKnownLocation);
        }
        if (delegate) {
            delegate.lastKnownLocation = '/dashboard';
            await team.save();
        }
        res.render('dashboard', { title: 'Dashboard', user: req.user, basePath });
    } catch (error) {
        res.render('dashboard', { title: 'Dashboard', user: req.user, basePath });
    }
});
router.get('/waiting', protectPlayerRoute, trackLocation, (req, res) => res.render('waiting', { title: 'Waiting for Team', user: req.user, basePath }));
router.get('/post-mission-wait', protectPlayerRoute, trackLocation, (req, res) => res.render('post_mission_waiting', { title: 'Mission Complete - Awaiting Team', user: req.user, basePath }));
router.get('/webex', protectPlayerRoute, trackLocation, (req, res) => res.render('webex', { title: 'Final Challenge', user: req.user, basePath }));
router.get('/role/cyber', protectPlayerRoute, authorizeRole('cyber'), trackLocation, (req, res) => res.render('role_cyber', { title: 'CyberSecurity Expert', user: req.user, basePath }));
router.get('/role/eng', protectPlayerRoute, authorizeRole('eng'), trackLocation, (req, res) => res.render('role_engineer', { title: 'Engineer', user: req.user, basePath }));
router.get('/role/opera', protectPlayerRoute, authorizeRole('opera'), trackLocation, (req, res) => res.render('role_operations', { title: 'Operations Expert', user: req.user, basePath }));

// ======================= ADMIN ROUTES (on router) =======================
router.get('/admin', (req, res) => res.render('admin_login', { title: 'Admin Login', error: null, basePath }));
router.post('/admin/login', async (req, res) => {
    ...
    if (username === ADMIN_USER && password === ADMIN_PASS) {
        ...
        res.render('admin_login', { title: 'Admin Login', error: 'Invalid username or password.', basePath });
    }
});
router.get('/admin/dashboard', protectAdminRoute, async (req, res) => {
    try {
        const teams = await User.find({});
        res.render('admin', { title: 'Admin Dashboard', teams: teams, basePath });
    } catch (error) {
...
router.get('/admin/logout', (req, res) => {
    req.session.destroy(err => {
        if (err) return res.redirect(`${basePath}/admin/dashboard`);
